...
void MyViewer::updateAccurateMeanCurvature() {
  for (auto vertex : mesh.vertices()) {
    std::pair<double, double> uv = uvs[vertex.idx()];

    double u = uv.first;
    double v = uv.second;

    mesh.data(vertex).mean = coons.calculateMean(u, v);
  }
}
...
void MyViewer::updateAlmostBoundary(){
    for (auto f : mesh.faces()) {
        bool isNearBoundary = false;

        for (auto v : mesh.fv_range(f)) {
            for (auto ff : mesh.vf_range(v)) {
                if(mesh.is_boundary(ff, vertexBoundary)) {
                    isNearBoundary = true;
                }
            }
        }

        almostBoundaryFaces[f] = isNearBoundary && !mesh.is_boundary(f, vertexBoundary);
...
Vec MyViewer::accurateMeanMapColor(double u, double v) const {

  return HSV2RGB({1, 1, 1});
}
...
void MyViewer::updateMesh(bool update_mean_range) {
  ...
  if (model_type == ModelType::COONS_SURFACE)
    generateCoonsMesh(50);

  mesh.request_face_normals(); mesh.request_vertex_normals();
  mesh.update_face_normals();
#ifdef USE_JET_FITkTING
  mesh.update_vertex_normals();
  updateWithJetFit(20);
#else // !USE_JET_FITTING
  updateVertexNormals();
  if(model_type == ModelType::COONS_SURFACE && visualization == Visualization::ACCURATE_MEAN)
      updateAccurateMeanCurvature();
  else
      updateMeanCurvature();
#endif
...
bool MyViewer::openCoons(const std::string &filename, bool update_view) {
  std::vector<size_t> cpN(4);
  std::vector<Point> cpS;

  try {
    std::ifstream f(filename.c_str());
    f.exceptions(std::ios::failbit | std::ios::badbit);
    f >> cpN[0] >> cpN[1] >> cpN[2] >> cpN[3];

    for(int i=0; i<4; i++) {
        for(int j=0; j<cpN[i]; j++) {
            double x, y, z;
            f >> x >> y >> z;
            cpS.push_back({x,y,z});
        }
    ...
    coons = CoonsSurface::createCoonsSurface3(cpS, cpN);
...
bool MyViewer::saveCoons(const std::string &filename) {
  ...
  try {
    std::ofstream f(filename.c_str());
    f.exceptions(std::ios::failbit | std::ios::badbit);
    f << coons.controlPointsSizes[0]
            << ' ' << coons.controlPointsSizes[1]
            << ' ' << coons.controlPointsSizes[2]
            << ' ' << coons.controlPointsSizes[3]
            << std::endl;

    for (auto p : coons.controlPoints)
...
void MyViewer::draw() {
  ...
  if (show_solid || show_wireframe) {
    ...
    for (auto f : mesh.faces()) {
      ...
      bool isAlmostBoundary = almostBoundaryFaces[f];

      for (auto v : mesh.fv_range(f)) {
        if (visualization == Visualization::MEAN)
          glColor3dv(meanMapColor(mesh.data(v).mean));
        if (visualization == Visualization::ACCURATE_MEAN)
          glColor3dv(meanMapColor(mesh.data(v).mean));
        else if (visualization == Visualization::SLICING)
          glTexCoord1d(mesh.point(v) | slicing_dir * slicing_scaling);
        else if (visualization == Visualization::ALMOSTBOUNDARY ){
            if(mesh.is_boundary(f))
                glColor3d(1.0, 1.0, 1.0);
            //else if(mesh.is_boundary(f, true))
            //    glColor3d(0.5, 0.0, 0.0);
            else if(isAlmostBoundary)
                glColor3dv(HSV2RGB({0, 1, 1}));
            else
                glColor3d(1.0, 1.0, 1.0);
        }
...
void MyViewer::drawCoonsControlNet() const {
  glDisable(GL_LIGHTING);
  glLineWidth(3.0);
  glColor3d(0.3, 0.3, 1.0);

  std::vector<std::shared_ptr<Point>> cps = coons.controlPoints;
  std::vector<Vec> allCP;

  glBegin(GL_LINE_STRIP);
  for(auto p : cps){
      glVertex3dv(Vec(p->x, p->y, p->z));
      allCP.push_back(Vec(p->x, p->y, p->z));
  }

  if(cps.size() > 0) {
      glVertex3dv(Vec(cps[0]->x, cps[0]->y, cps[0]->z));
  }
  glEnd();
...
void MyViewer::drawWithNames() {
  ...
  case ModelType::COONS_SURFACE:
    if (!show_control_points)
      return;
    for (size_t i = 0, ie = coons.controlPoints.size(); i < ie; ++i) {
      Vec const p = *coons.controlPoints[i];
      glPushName(i);
      glRasterPos3fv(p);
      glPopName();
    }
...
void MyViewer::postSelection(const QPoint &p) {
  ...
  if (model_type == ModelType::COONS_SURFACE)
    axes.position = *coons.controlPoints[sel];
...
void MyViewer::keyPressEvent(QKeyEvent *e) {
  if ((e->modifiers() & Qt::ShiftModifier) && (e->key() & Qt::Key_M)) {
    visualization = Visualization::ACCURATE_MEAN;
    updateMesh(true);
    update();
  }
...
void MyViewer::generateCoonsMesh(size_t resolution) {
  mesh.clear();
  std::vector<MyMesh::VertexHandle> handles, tri;
  ...
  for (size_t i = 0; i < resolution; ++i) {
    ...
    for (size_t j = 0; j < resolution; ++j) {
      ...
      Point p = coons.evaluate(u, v);
      Vector vec{p.x, p.y, p.z};
      auto handle = mesh.add_vertex(vec);
      uvs[handle.idx()] = {u, v};
...
void MyViewer::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (model_type == ModelType::COONS_SURFACE)
    *coons.controlPoints[selected_vertex] = axes.position;
