...
void MyWindow::save() {
    switch(viewer->getModelType()) {
    case ModelType::NONE:
        QMessageBox::warning(this, tr("Empty object"), tr("There is no object in the scene!"));
        break;
    case ModelType::MESH:
        QMessageBox::warning(this, tr("Not supported file"), tr("Mesh file is not supported!"));
        break;
    case ModelType::BEZIER_SURFACE: {
        auto filenameBezier =  QFileDialog::getSaveFileName(this, tr("Save File"), last_directory, tr("Bézier surface (*.bzr);;"));

        if(filenameBezier.isEmpty())
            return;

        last_directory = QFileInfo(filenameBezier).absolutePath();

        if (!viewer->saveBezier(filenameBezier.toUtf8().data()))
            QMessageBox::warning(this, tr("Cannot save file"), tr("Could not save file: ") + filenameBezier + ".");

        break;
    }
    case ModelType::COONS_SURFACE: {
        auto filenameCoons = QFileDialog::getSaveFileName(this, tr("Save File"), last_directory, tr("Coons surface (*.cns);;"));

        if(filenameCoons.isEmpty())
            return;

        last_directory = QFileInfo(filenameCoons).absolutePath();

        if (!viewer->saveCoons(filenameCoons.toUtf8().data()))
            QMessageBox::warning(this, tr("Cannot save file"),
                             tr("Could not save file: ") + filenameCoons + ".");
        break;
    }
    }
